"""
ml_model.py  — TopoAlpha v2 (Corrected)
"""
...
class TopoBooster:
    ...
    def __init__(self):
        ...
        self._smooth_probs: np.ndarray = np.array([1.0, 0.0, 0.0])

    def _build_ensemble(self):
        gbm = GradientBoostingClassifier(
            n_estimators=200, learning_rate=0.04, max_depth=4, subsample=0.8,
            min_samples_leaf=20, random_state=42
        )
        rf = RandomForestClassifier(
            n_estimators=200, max_depth=6, min_samples_leaf=20, max_features="sqrt",
    ...
    def _build_features(self, prices: list, stress_history: list, obi_history: list, prices_htf=None) -> pd.DataFrame:
        ...
        df["stress_zscore"] = (
                (df["stress"] - df["stress"].rolling(30).mean()) / (df["stress"].rolling(30).std() + 1e-10))
